...
class Thermochemistry:
    ...
    def get_enthalpy_of_formation(self):
        return self.usr_np.array([
            2.11781407e+01,  6.59282584e-09, -1.58739241e+02, -4.45870455e+01,
            -9.75509534e+01,  5.35780531e-09,  5.76815988e-04
        ])
    ...
    def get_mole_fractions(self, mass_fractions):
        mmw = self.get_mix_molecular_weight(mass_fractions)
        return self._pyro_make_array([
                self.iwts[0] * mass_fractions[0] * mmw,
                self.iwts[1] * mass_fractions[1] * mmw,
                self.iwts[2] * mass_fractions[2] * mmw,
                self.iwts[3] * mass_fractions[3] * mmw,
                self.iwts[4] * mass_fractions[4] * mmw,
                self.iwts[5] * mass_fractions[5] * mmw,
                self.iwts[6] * mass_fractions[6] * mmw,
            ])
    ...
    def get_mixture_viscosity_mixavg(self, temperature, mass_fractions):
        mole_fractions = self.get_mole_fractions(mass_fractions)
    ...
    def get_species_mass_diffusivities_mixavg(self, temperature, pressure,
        ...
        mix_rule_f = self._pyro_make_array([
              (self.usr_np.sqrt(temperature)*temperature/pressure)*self.usr_np.where(
                  self.usr_np.greater(1.0-mole_fractions[0], threshold),
                  (1.0 + -1*((mole_fractions[0]*28.054) / mmw)) / (mole_fractions[0] / diff_ij[0, 0] + mole_fractions[1] / diff_ij[0, 1] + mole_fractions[2] / diff_ij[0, 2] + mole_fractions[3] / diff_ij[0, 3] + mole_fractions[4] / diff_ij[0, 4] + mole_fractions[5] / diff_ij[0, 5] + mole_fractions[6] / diff_ij[0, 6] + -1*(mole_fractions[0] / diff_ij[0, 0])),
                  diff_ij[0, 0],
              ),
              (self.usr_np.sqrt(temperature)*temperature/pressure)*self.usr_np.where(
                  self.usr_np.greater(1.0-mole_fractions[1], threshold),
                  (1.0 + -1*((mole_fractions[1]*31.998) / mmw)) / (mole_fractions[0] / diff_ij[1, 0] + mole_fractions[1] / diff_ij[1, 1] + mole_fractions[2] / diff_ij[1, 2] + mole_fractions[3] / diff_ij[1, 3] + mole_fractions[4] / diff_ij[1, 4] + mole_fractions[5] / diff_ij[1, 5] + mole_fractions[6] / diff_ij[1, 6] + -1*(mole_fractions[1] / diff_ij[1, 1])),
                  diff_ij[1, 1],
              ),
              (self.usr_np.sqrt(temperature)*temperature/pressure)*self.usr_np.where(
                  self.usr_np.greater(1.0-mole_fractions[2], threshold),
                  (1.0 + -1*((mole_fractions[2]*44.009) / mmw)) / (mole_fractions[0] / diff_ij[2, 0] + mole_fractions[1] / diff_ij[2, 1] + mole_fractions[2] / diff_ij[2, 2] + mole_fractions[3] / diff_ij[2, 3] + mole_fractions[4] / diff_ij[2, 4] + mole_fractions[5] / diff_ij[2, 5] + mole_fractions[6] / diff_ij[2, 6] + -1*(mole_fractions[2] / diff_ij[2, 2])),
                  diff_ij[2, 2],
              ),
              (self.usr_np.sqrt(temperature)*temperature/pressure)*self.usr_np.where(
                  self.usr_np.greater(1.0-mole_fractions[3], threshold),
                  (1.0 + -1*((mole_fractions[3]*28.009999999999998) / mmw)) / (mole_fractions[0] / diff_ij[3, 0] + mole_fractions[1] / diff_ij[3, 1] + mole_fractions[2] / diff_ij[3, 2] + mole_fractions[3] / diff_ij[3, 3] + mole_fractions[4] / diff_ij[3, 4] + mole_fractions[5] / diff_ij[3, 5] + mole_fractions[6] / diff_ij[3, 6] + -1*(mole_fractions[3] / diff_ij[3, 3])),
                  diff_ij[3, 3],
              ),
              (self.usr_np.sqrt(temperature)*temperature/pressure)*self.usr_np.where(
                  self.usr_np.greater(1.0-mole_fractions[4], threshold),
                  (1.0 + -1*((mole_fractions[4]*18.015) / mmw)) / (mole_fractions[0] / diff_ij[4, 0] + mole_fractions[1] / diff_ij[4, 1] + mole_fractions[2] / diff_ij[4, 2] + mole_fractions[3] / diff_ij[4, 3] + mole_fractions[4] / diff_ij[4, 4] + mole_fractions[5] / diff_ij[4, 5] + mole_fractions[6] / diff_ij[4, 6] + -1*(mole_fractions[4] / diff_ij[4, 4])),
                  diff_ij[4, 4],
              ),
              (self.usr_np.sqrt(temperature)*temperature/pressure)*self.usr_np.where(
                  self.usr_np.greater(1.0-mole_fractions[5], threshold),
                  (1.0 + -1*((mole_fractions[5]*2.016) / mmw)) / (mole_fractions[0] / diff_ij[5, 0] + mole_fractions[1] / diff_ij[5, 1] + mole_fractions[2] / diff_ij[5, 2] + mole_fractions[3] / diff_ij[5, 3] + mole_fractions[4] / diff_ij[5, 4] + mole_fractions[5] / diff_ij[5, 5] + mole_fractions[6] / diff_ij[5, 6] + -1*(mole_fractions[5] / diff_ij[5, 5])),
                  diff_ij[5, 5],
              ),
              (self.usr_np.sqrt(temperature)*temperature/pressure)*self.usr_np.where(
                  self.usr_np.greater(1.0-mole_fractions[6], threshold),
                  (1.0 + -1*((mole_fractions[6]*28.014) / mmw)) / (mole_fractions[0] / diff_ij[6, 0] + mole_fractions[1] / diff_ij[6, 1] + mole_fractions[2] / diff_ij[6, 2] + mole_fractions[3] / diff_ij[6, 3] + mole_fractions[4] / diff_ij[6, 4] + mole_fractions[5] / diff_ij[6, 5] + mole_fractions[6] / diff_ij[6, 6] + -1*(mole_fractions[6] / diff_ij[6, 6])),
                  diff_ij[6, 6],
              ),
              ])
        return mix_rule_f
